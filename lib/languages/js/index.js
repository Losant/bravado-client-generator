...
const generateApiJs = async (api, options) => {
  ...
  const globalParams = [ ...DEFAULT_GLOBAL_PARAMS, ...(utils.pluckParamFields(api, DEFAULT_GLOBAL_PARAMS_NAMES)) ];
  const globalParamNames = new Set(globalParams.map(({ name }) => name));
  const apiSource = templates.api({ api, globalParams, globalParamNames, options });
