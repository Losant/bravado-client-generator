...
const debug = _debug('client-gen:javascript');

const DEFAULT_GLOBAL_PARAMS = [
  { name: '_actions', in: 'query', type: 'boolean' },
  { name: '_links', in: 'query', type: 'boolean' },
  { name: '_embedded', in: 'query', type: 'boolean' }
];

const DEFAULT_GLOBAL_PARAMS_NAMES = new Set(DEFAULT_GLOBAL_PARAMS.map(({ name }) => name));
...
const generateApiJs = async (api, options) => {
  ...
  const globalParams = [ ...DEFAULT_GLOBAL_PARAMS, ...(pluckParamFields(api, DEFAULT_GLOBAL_PARAMS_NAMES)) ];
  const globalPath = path.resolve(options.output, 'lib/api/constants/globalParams.json');
  await fs.outputFile(globalPath, JSON.stringify(globalParams));
  const globalParamNames = new Set(globalParams.map(({ name }) => name));
  const apiSource = templates.api({ api, globalParamNames, options });
...
const generateSchemaDoc = async (api, options) => {
  ...
  const infoFilePath = path.resolve(options.output, 'lib/schemas/apiInfo.json');
  debug('writing lib/schemas/apiInfo.json');
  await fs.outputFile(infoFilePath, stableStringify(api.resources));

  const expFilePath = path.resolve(options.output, 'lib/schemas/apiExamples.json');
  debug('writing lib/schemas/apiExamples.json');
  await fs.outputFile(expFilePath, stableStringify(api.examples));
};
