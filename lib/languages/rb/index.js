...
var utilsTemplate = handlebars.compile(
...
function generateVersionRb (api, options) {
  ...
  var file = path.resolve(options.output, name);
  debug('generating ' + name);
  var versionSource = versionTemplate({ api: api, options: options });
  debug('writing ' + name);
  fs.outputFileSync(file, versionSource);
}

function generateUtilsRb (api, options) {
  var name = api.info.baseDir + '/utils.rb';
  var file = path.resolve(options.output, name);
  debug('generating ' + name);
  var utilsSource = utilsTemplate({ api: api, options: options });
  debug('writing ' + name);
