...
import mkdocsHelpers from '../../mkdocs/helpers/index.js';

const helpers = { ...commonHelpers, ...mkdocsHelpers };
...
helpers.niceResourceName = function(name) {
  ...
  } else if (name.startsWith('Application Key')) {
    return name.replace('Application Key', 'Access Key');
  } else if (name.startsWith('Application Certificate')) {
    return name.replace('Application Certificate', 'Device Certificate');
