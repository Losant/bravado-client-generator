...
helpers.generateCurlExample = function(api, resourceName, actionName) {
  ...
  params.forEach(function(param) {
    if (param.private || !param.required) {
      return;
    }
...
helpers.definedParams = function(
  api,
  resource,
  action,
  includePrivate,
    ...
    definedParams = definedParams.concat([
      {
        ...
        in: 'query',
        type: 'boolean',
        description: 'Return resource actions in response',
        default: false
      },
      {
        private: true,
        ...
        in: 'query',
        type: 'boolean',
        description: 'Return resource link in response',
        default: true
      },
      {
        private: true,
        ...
        in: 'query',
        type: 'boolean',
        description: 'Return embedded resources in response',
        default: true
      }
    ]);
  }
  ...
  definedParams.forEach(function(param) {
    if (!includePrivate && param.private) {
      return;
    }
    if (param.name === 'losantdomain') {
      return;
    }
