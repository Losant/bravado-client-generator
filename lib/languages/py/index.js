...
var resourceDocTemplate = handlebars.compile(
  fs.readFileSync(path.resolve(__dirname, './templates/resource.rst.tpl'), { encoding: 'utf8' })
);
var schemasDocTemplate = handlebars.compile(
  fs.readFileSync(path.resolve(__dirname, './templates/schemas.rst.tpl'), { encoding: 'utf8' })
);
...
function generateResourceDoc (api, resourceName, options) {
  var resource = api.resources[resourceName];
  var file = path.resolve(options.output, 'docs/' + resourceName + '.rst');
...
function generateSchemaDoc (api, options) {
  var file = path.resolve(options.output, 'docs/_schemas.rst');
