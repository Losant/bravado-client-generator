...
var helpers = {
  ...
  hasParams: function (api, resource, action, includeHidden, options) {
    ...
    for(var i=0; i<definedParams.length; i++){
      if(includeHidden || !definedParams[i].private)
      { return true; }
    }
    return false;
  },
  ...
  sseResponseComment: function (options, eventName, eventInfo) {
    ...
    var schema = eventInfo.schema && eventInfo.schema.$ref ?
      '(' + url.resolve(options.root, eventInfo.schema.$ref) + ')' :
  ...
  sseResponseDoc: function (api, eventName, eventInfo) {
    ...
    if (eventInfo.schema && eventInfo.schema.$ref) {
      var wrapper = api.schemaMap[eventInfo.schema.$ref];
      type = '[' + wrapper.title + '](_schemas.md#' + helpers.dasherize(wrapper.title) + ')';
    }
    return util.format('| %s | %s | %s |', eventName, type, eventInfo.description);
  },

  hasAuthScopes: function (api, resource, action, options) {
    var validScopes = [].concat(
  ...
  validAuthScopes: function (api, resource, action, options) {
    var validScopes = [].concat(
      Array.isArray(api.auth) ? api.auth : [],
      Array.isArray(resource.auth) ? resource.auth: [],
      Array.isArray(action.auth) ? action.auth: []
    ...
    Object.keys(api.authGroups).forEach((authGroup) => {
      ...
      if(_.some(api.authGroups[authGroup], (scope) => { return validScopes.indexOf(scope) >= 0; }))
      { validScopes.push(authGroup); }
    });

    return _.uniq(validScopes).sort();
  },

  arrayToTextList: function(array, options) {
