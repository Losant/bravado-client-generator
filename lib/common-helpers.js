...
const helpers = {
  ...
  buildParams: function(api, resource, action) {
    var definedParams = [].concat(
      api.params || [], action.params || [], resource.params || []
    );
    definedParams = definedParams.map(({ name, in: asIn, required, type }) => {
      return { name, required, in: asIn, type }
    });
    return JSON.stringify(definedParams);
  },
  ...
  parameterComment: function(options, param) {
    ...
    const schema = param.schema && param.schema.$ref ?
      ` (${new URL(param.schema.$ref, options.root)})` : '';
  ...
  responseComment: function(options, code, response) {
    ...
    if (response.schema && response.schema.$ref) {
      schema = `(${new URL(response.schema.$ref, options.root)})`;
  ...
  sseResponseComment: function(options, eventName, eventInfo) {
    ...
    if (eventInfo.schema && eventInfo.schema.$ref) {
      schema = `(${new URL(eventInfo.schema.$ref, options.root)})`;
  ...
  parameterDoc: function(api, param, schemaPath, options) {
    if (options === undefined) { schemaPath = options; }
    ...
    if (param.schema && param.schema.$ref) {
      ...
      if (schemaPath) {
        type = `[${wrapper.title}](${schemaPath}/${wrapper.file}.json)`;
      } else {
  ...
  responseDoc: function(api, code, response, schemaPath, options) {
    if (options === undefined) { schemaPath = options; }
  ...
  sseResponseDoc: function(api, eventName, eventInfo, schemaPath, options) {
    if (options === undefined) { schemaPath = options; }
    if (!eventInfo.description) { throw new Error(`Missing SSE Event Description: ${eventName}, ${eventInfo}`); }
  ...
  arrayToCodeList: function(array) {
    return array.map((v) => `\`${v}\``).join(', ').replace(/, ([^,]*)$/, ', or $1');
  },
